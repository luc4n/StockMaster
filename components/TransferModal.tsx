    ...
    const fetchOriginStock = async () => {
        ...
        if (data) {
            ...
            const getProductName = (p: any) => {
                if (Array.isArray(p)) return p[0]?.name || 'Produto Desconhecido';
                return p?.name || 'Produto Desconhecido';
            };

            data.forEach((d: any) => {
                const pId = d.product_id;
                if (!stockMap[pId]) {
                    stockMap[pId] = { id: pId, name: getProductName(d.products), quantity: 0 };
                }

                // Handling all types that affect possession
                if (d.type === 'Saída' || d.type === 'Transferência (Entrada)') {
                    stockMap[pId].quantity += (d.quantity || 0);
